...
class MainWidget(QWidget):
    def __init__(self):
        ...
        # Add right selection area to the main layout
        right_widget = QWidget()
        right_widget.setLayout(self.right_selection_area)
    ...
    def start_new_gate(self, col, row, gate):
        if gate.num_controls + gate.num_targets > 1:
            ...
            self.gate_positions[row][col] = '*'  # Mark control position
    ...
    def add_target_to_gate(self, col, row, gate):
        ...
        if len(gate_info['control']) < gate.num_controls:
            ...
            self.gate_positions[row][col] = gate_info['type']  # Mark target position
        ...
        for col, gate_info in self.active_gates.items():
            if gate_info['type'] == gate_name:
                if old_row in gate_info['control']:
                    self.gate_positions[old_row][old_col] = ""  # Clear old position
                    self.gate_positions[new_row][new_col] = '*'  # Mark new control position
                    gate_info['control'].remove(old_row)
                    gate_info['control'].append(new_row)
                if old_row in gate_info['target']:
                    self.gate_positions[old_row][old_col] = ""  # Clear old position
                    self.gate_positions[new_row][new_col] = self.gate_info['type']  # Mark new target position
